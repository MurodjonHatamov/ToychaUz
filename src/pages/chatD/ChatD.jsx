...
function ChatD({setNotifications, notifications }) {
  ...
  const fetchMarketChat = async (marketId) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:2277/deliver/chat/${marketId}`, {
        method: 'GET',
        headers: { 
          'accept': '*/*',
          'Content-Type': 'application/json'
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:2277/deliver/send-message/${selectedMarket._id}`, {
        method: 'POST',
        headers: { 
          'accept': '*/*',
          'Content-Type': 'application/json'
  ...
  const getLastMessage = (marketId) => {
    const marketMessages = allMessages.filter(msg => 
            ...
            {loading.markets ? (
              ...
              filteredMarkets.map((market) => {
                ...
                return (
                  <div
                    key={market._id}
                    className={`${styles.marketItem} ${isActive ? styles.marketItemActive : ''}`}
              ...
              <div className={styles.chatHeader}>
                <div className={styles.chatPartner}>
                  ...
                  <div className={styles.partnerInfo}>
                    <div className={styles.partnerName}>{selectedMarket.name}</div>
                    <div className={styles.partnerPhone}>{selectedMarket.phone}</div>
                  </div>
                </div>
              </div>
                        ...
                        <React.Fragment key={message._id}>
                          ...
                          {showDate && (
                            <div className={styles.dateDivider}>
                              {isToday(message.createdAt) ? 'Bugun' : formatDate(message.createdAt)}
                            </div>
                          )}
                          
                          {/* Xabar */}
                          <div className={`${styles.message} ${isDeliver ? styles.messageOutgoing : styles.messageIncoming}`}>
                              ...
                              <div className={styles.messageTime}>
                                ...
                                {isDeliver && (
                                  <span className={styles.statusIcon}>
                                    {message.status === 'viewed' ? <FaCheckDouble /> : <FaCheck />}
                                  </span>
                                )}
                              </div>
                            </div>
